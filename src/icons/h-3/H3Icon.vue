...
// AI Generated Motion Component for: h-3
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for h-3 (tabler). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-h-3 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-h-3 3s ease-in-out infinite alternate;
}
</style>