...
// AI Generated Motion Component for: route-alt-left
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for route-alt-left (tabler). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (kinetic-push) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-route-alt-left {
...
@keyframes motion-ai-idle-route-alt-left {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-route-alt-left 0.25s cubic-bezier(0.25, 1, 0.5, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-route-alt-left 3s ease-in-out infinite alternate;
}
</style>