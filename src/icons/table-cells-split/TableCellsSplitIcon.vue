...
// AI Generated Motion Component for: table-cells-split
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for table-cells-split (lucide). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-table-cells-split {
...
@keyframes motion-ai-idle-table-cells-split {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-table-cells-split 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-table-cells-split 3s ease-in-out infinite alternate;
}
</style>