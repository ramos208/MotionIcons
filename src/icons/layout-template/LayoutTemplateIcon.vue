...
// AI Generated Motion Component for: layout-template
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for layout-template (lucide). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-layout-template {
...
@keyframes motion-ai-idle-layout-template {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-layout-template 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-layout-template 3s ease-in-out infinite alternate;
}
</style>