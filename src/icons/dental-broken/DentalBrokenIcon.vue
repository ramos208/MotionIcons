...
// AI Generated Motion Component for: dental-broken
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for dental-broken (tabler). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-dental-broken {
...
@keyframes motion-ai-idle-dental-broken {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-dental-broken 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-dental-broken 3s ease-in-out infinite alternate;
}
</style>