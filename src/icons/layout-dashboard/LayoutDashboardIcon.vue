...
// AI Generated Motion Component for: layout-dashboard
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for layout-dashboard (tabler). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-layout-dashboard {
...
@keyframes motion-ai-idle-layout-dashboard {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-layout-dashboard 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-layout-dashboard 3s ease-in-out infinite alternate;
}
</style>