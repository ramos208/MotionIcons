...
// AI Generated Motion Component for: camera-check
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for camera-check (tabler). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-camera-check {
...
@keyframes motion-ai-idle-camera-check {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-camera-check 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-camera-check 3s ease-in-out infinite alternate;
}
</style>