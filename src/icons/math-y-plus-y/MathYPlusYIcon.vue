...
// AI Generated Motion Component for: math-y-plus-y
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for math-y-plus-y (tabler). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-math-y-plus-y {
...
@keyframes motion-ai-idle-math-y-plus-y {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-math-y-plus-y 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-math-y-plus-y 3s ease-in-out infinite alternate;
}
</style>