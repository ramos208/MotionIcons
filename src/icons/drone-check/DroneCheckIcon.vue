...
// AI Generated Motion Component for: drone-check
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for drone-check (iconoir). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-drone-check {
...
@keyframes motion-ai-idle-drone-check {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-drone-check 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-drone-check 3s ease-in-out infinite alternate;
}
</style>