...
// AI Generated Motion Component for: map-pin-check-inside
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for map-pin-check-inside (lucide). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-map-pin-check-inside {
...
@keyframes motion-ai-idle-map-pin-check-inside {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-map-pin-check-inside 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-map-pin-check-inside 3s ease-in-out infinite alternate;
}
</style>