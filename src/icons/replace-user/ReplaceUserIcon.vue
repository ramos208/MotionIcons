...
// AI Generated Motion Component for: replace-user
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for replace-user (tabler). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-replace-user {
...
@keyframes motion-ai-idle-replace-user {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-replace-user 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-replace-user 3s ease-in-out infinite alternate;
}
</style>