...
// AI Generated Motion Component for: cloudera
// Category: weather
// Physics Presets: Dynamic kinetic adaptation for cloudera (simpleicons). Selected semantic motion: turbulence with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-idle-cloudera {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-cloudera 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-cloudera 3s ease-in-out infinite alternate;
}
</style>