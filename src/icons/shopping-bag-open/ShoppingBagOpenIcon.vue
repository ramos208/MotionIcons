...
// AI Generated Motion Component for: shopping-bag-open
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for shopping-bag-open (phosphor). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-shopping-bag-open {
...
@keyframes motion-ai-idle-shopping-bag-open {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-shopping-bag-open 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-shopping-bag-open 3s ease-in-out infinite alternate;
}
</style>