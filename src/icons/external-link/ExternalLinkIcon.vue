...
// AI Generated Motion Component for: external-link
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for external-link (tabler). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-external-link {
...
@keyframes motion-ai-idle-external-link {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-external-link 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-external-link 3s ease-in-out infinite alternate;
}
</style>