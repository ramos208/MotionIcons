...
// AI Generated Motion Component for: layout-left
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for layout-left (hugeicons). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (kinetic-push) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-layout-left {
...
@keyframes motion-ai-idle-layout-left {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-layout-left 0.25s cubic-bezier(0.25, 1, 0.5, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-layout-left 3s ease-in-out infinite alternate;
}
</style>