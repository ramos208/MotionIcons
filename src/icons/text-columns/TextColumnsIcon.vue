...
// AI Generated Motion Component for: text-columns
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for text-columns (phosphor). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-text-columns {
...
@keyframes motion-ai-idle-text-columns {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-text-columns 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-text-columns 3s ease-in-out infinite alternate;
}
</style>