...
// AI Generated Motion Component for: media-image
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for media-image (iconoir). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-media-image {
...
@keyframes motion-ai-idle-media-image {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-media-image 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-media-image 3s ease-in-out infinite alternate;
}
</style>