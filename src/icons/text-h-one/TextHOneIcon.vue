...
// AI Generated Motion Component for: text-h-one
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for text-h-one (phosphor). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-text-h-one {
...
@keyframes motion-ai-idle-text-h-one {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-text-h-one 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-text-h-one 3s ease-in-out infinite alternate;
}
</style>