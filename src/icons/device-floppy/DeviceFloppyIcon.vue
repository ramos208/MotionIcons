...
// AI Generated Motion Component for: device-floppy
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for device-floppy (tabler). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-device-floppy {
...
@keyframes motion-ai-idle-device-floppy {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-device-floppy 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-device-floppy 3s ease-in-out infinite alternate;
}
</style>