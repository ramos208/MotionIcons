...
// AI Generated Motion Component for: filter-2-cancel
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for filter-2-cancel (tabler). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-filter-2-cancel {
...
@keyframes motion-ai-idle-filter-2-cancel {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-filter-2-cancel 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-filter-2-cancel 3s ease-in-out infinite alternate;
}
</style>