...
// AI Generated Motion Component for: aiohttp
// Category: ai
// Physics Presets: Dynamic kinetic adaptation for aiohttp (simpleicons). Selected semantic motion: neural-pulse with custom idle (neural-pulse) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-idle-aiohttp {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-aiohttp 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-neural-pulse {
  animation: motion-ai-idle-aiohttp 2s ease-in-out infinite alternate;
}
</style>