...
// AI Generated Motion Component for: grid-2x2-x
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for grid-2x2-x (lucide). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-grid-2x2-x {
...
@keyframes motion-ai-idle-grid-2x2-x {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-grid-2x2-x 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-grid-2x2-x 3s ease-in-out infinite alternate;
}
</style>