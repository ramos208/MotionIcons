...
// AI Generated Motion Component for: file-type-ts
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for file-type-ts (tabler). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-file-type-ts {
...
@keyframes motion-ai-idle-file-type-ts {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-file-type-ts 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-file-type-ts 3s ease-in-out infinite alternate;
}
</style>