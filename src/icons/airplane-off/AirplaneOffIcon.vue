...
// AI Generated Motion Component for: airplane-off
// Category: ai
// Physics Presets: Dynamic kinetic adaptation for airplane-off (iconoir). Selected semantic motion: neural-pulse with custom idle (neural-pulse) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-airplane-off {
...
@keyframes motion-ai-idle-airplane-off {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-airplane-off 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-neural-pulse {
  animation: motion-ai-idle-airplane-off 2s ease-in-out infinite alternate;
}
</style>