...
// AI Generated Motion Component for: frame-minus-in
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for frame-minus-in (iconoir). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-frame-minus-in {
...
@keyframes motion-ai-idle-frame-minus-in {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-frame-minus-in 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-frame-minus-in 3s ease-in-out infinite alternate;
}
</style>