...
// AI Generated Motion Component for: cpu-2
// Category: ai
// Physics Presets: Dynamic kinetic adaptation for cpu-2 (tabler). Selected semantic motion: neural-pulse with custom idle (neural-pulse) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-idle-cpu-2 {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-cpu-2 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-neural-pulse {
  animation: motion-ai-idle-cpu-2 2s ease-in-out infinite alternate;
}
</style>