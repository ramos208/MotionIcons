...
// AI Generated Motion Component for: server-crash
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for server-crash (lucide). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-server-crash {
...
@keyframes motion-ai-idle-server-crash {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-server-crash 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-server-crash 3s ease-in-out infinite alternate;
}
</style>