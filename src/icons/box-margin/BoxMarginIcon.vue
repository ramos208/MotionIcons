...
// AI Generated Motion Component for: box-margin
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for box-margin (tabler). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-box-margin {
...
@keyframes motion-ai-idle-box-margin {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-box-margin 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-box-margin 3s ease-in-out infinite alternate;
}
</style>