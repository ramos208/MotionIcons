...
// AI Generated Motion Component for: droplet-pause
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for droplet-pause (tabler). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-droplet-pause {
...
@keyframes motion-ai-idle-droplet-pause {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-droplet-pause 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-droplet-pause 3s ease-in-out infinite alternate;
}
</style>