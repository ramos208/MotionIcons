...
// AI Generated Motion Component for: i18next
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for i18next (simpleicons). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-idle-i18next {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-i18next 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-i18next 3s ease-in-out infinite alternate;
}
</style>