...
// AI Generated Motion Component for: time-duration-off
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for time-duration-off (tabler). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-time-duration-off {
...
@keyframes motion-ai-idle-time-duration-off {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-time-duration-off 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-time-duration-off 3s ease-in-out infinite alternate;
}
</style>