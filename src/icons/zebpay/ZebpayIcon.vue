...
// AI Generated Motion Component for: zebpay
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for zebpay (simpleicons). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-idle-zebpay {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-zebpay 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-zebpay 3s ease-in-out infinite alternate;
}
</style>