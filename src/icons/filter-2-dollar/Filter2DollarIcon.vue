...
// AI Generated Motion Component for: filter-2-dollar
// Category: generic
// Physics Presets: Dynamic kinetic adaptation for filter-2-dollar (tabler). Selected semantic motion: radar with custom idle (ambient-floating) and interaction profile (elastic-scale) matched to SVG complexity (complex) and flow geometry.
...
@keyframes motion-ai-filter-2-dollar {
...
@keyframes motion-ai-idle-filter-2-dollar {
...
.ai-motion-icon-wrapper:hover .icon-geometry {
  animation: motion-ai-filter-2-dollar 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.anim-ambient-floating {
  animation: motion-ai-idle-filter-2-dollar 3s ease-in-out infinite alternate;
}
</style>